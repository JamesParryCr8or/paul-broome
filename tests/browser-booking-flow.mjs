...
await evaluate(`(() => { const i=document.querySelector('input[autocomplete="given-name"]'); const setter=Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,'value').set; setter.call(i,'Preview'); i.dispatchEvent(new Event('input',{bubbles:true})); document.querySelector('.details-form').requestSubmit(); })()`);
await waitFor("document.querySelector('input[autocomplete=\"organization\"]')");
await evaluate(`(() => { const i=document.querySelector('input[autocomplete="organization"]'); const setter=Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,'value').set; setter.call(i,'Preview Roofing Ltd'); i.dispatchEvent(new Event('input',{bubbles:true})); document.querySelector('.details-form').requestSubmit(); })()`);
...
await evaluate(`(() => { const set=(s,v)=>{const i=document.querySelector(s); const setter=Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,'value').set; setter.call(i,v); i.dispatchEvent(new Event('input',{bubbles:true}));}; set('input[autocomplete="email"]','preview@example.com'); set('input[autocomplete="tel"]','07700900123'); document.querySelector('.details-form').requestSubmit(); })()`);
...
console.log('PASS: hero, progressive name/company capture, ten visual questions, final contact capture, API preview completion and responsive overflow check.');
socket.close();
